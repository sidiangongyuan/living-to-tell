...
describe('app update store', () => {
  ...
  it('checks for updates automatically and supports dismissing the current release', async () => {
    ...
    vi.mocked(appApi.checkForUpdate).mockResolvedValue({
      ...
      release_url: 'https://example.test/releases/tag/living-to-tell-v0.1.10',
      ...
      cached: false,
      download_url: 'https://example.test/LivingToTell_0.1.10_x64-setup.exe',
    ...
    expect(store.latestVersion).toBe('0.1.10')

    store.dismissUpdate()
    expect(setItem).toHaveBeenCalledWith(UPDATE_DISMISSED_VERSION_KEY, '0.1.10')
