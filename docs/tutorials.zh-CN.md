...
## 5. AI 配置、文章修改与 AI Cards

![AI 与 AI Cards 教程](../tauri-mvp/docs/assets/tutorials/05-ai-cards.gif)

目标：配好一个可靠的默认档案，把文章修改交给明确勾选的多个模型对比，并用 AI Card 沉淀可复用上下文。

步骤：

1. 在 **设置 → AI** 查看档案健康状态，选定唯一默认档案，并打开三步配置向导。
2. 先做本地检查，只对明确勾选的档案发送最小真实测试。
3. 从文章或选区进入 **AI 修改**，选择润色、改写、扩写或续写，再勾选一个或多个档案。
4. 第一个模型成功后立即阅读结果，切换段落差异，再打开写回预览；选择模型越多，等待时间和供应商成本可能越高。
5. 在文章侧打开 **AI 对话** 抽屉，不离开正文也能讨论写法。
6. 进入 **AI Cards** 创建或维护风格、人物、场景卡；需要交给 AI 时使用 **复制为提示词**。

完成后你应该看到：AI 修改使用当前文章和实际勾选档案；结果只在你明确复制或确认写回后才产生变化，文章对话不会自动改正文。

安全提醒：本地配置存在不等于模型可用；真实测试可能使用 token 并产生费用。重连只查询任务状态，不重新发送 provider 请求。
