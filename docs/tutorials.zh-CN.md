...
2. 如果页面弹出互动教程，可以按 **下一步** 理解容器与正文节点、未编排文章、标题同步、章节拆分、看板拖动和导出；关闭后可在设置中重新显示。
3. 在 **书稿** 页先新增顶层分组/分部/辑/部分或章节，再在章节下放文章、场景或小节。
4. 选中章节，点 **新子项**，或把 **未编排文章** 放到当前节点下。已经直接关联正文的章节会先确认，再把正文移到第一个子项。
5. 未关联节点使用规划标题；关联后统一显示真实文章标题。详情区仍可维护类型、上级节点、状态、摘要和目标字数。
6. 切到 **看板**。同一列上下拖动用来安排“接下来先做什么”；跨列拖动同时改变当前卡状态和落点。看板优先级始终不改变书稿或导出顺序。
7. 切到 **导出**，按书稿结构导出正文，或导出仅供自己查看的规划文件。

完成后你应该看到：作品集是一棵清楚的“结构容器 → 正文节点”书稿树；章节下可放多篇文章，未进入树的文章会留在未编排区。
...
3. 从文章或选区进入 **AI 修改**，选择润色、改写、扩写或续写，再从 25 个目的预设中选择本轮方向。创作性目的会提示风险，视角、时态等使用结构化选项。
4. 在 **参考上下文** 中按需打开三个大选择器：文脉标本、AI Cards、文章便签。搜索、预览、暂存选择，最后确认；只查看不会附加。
5. 勾选一个或多个档案；每个模型收到同一份冻结文章、目的、控制参数和上下文快照。
6. 点击运行后直接进入独立结果工作区，查看本次启动历史、原文对照、双模型、段落差异和可选编辑副本，再明确写回。
7. 在文章侧打开 **AI 对话** 抽屉，不离开正文也能讨论写法。

完成后你应该看到：AI 修改只使用当前文章、实际勾选档案、明确目的和确认过的参考资料；配置与结果分开，只有明确写回才改变文章。

安全提醒：本地配置存在不等于模型可用；真实测试可能使用 token 并产生费用。重连只查询任务状态，不重新发送 provider 请求；切换文章会清空便签选择，避免串到另一篇正文。模型原稿在展示与原样写回前会保守整理意外空行，作者编辑副本保留自己安排的留白。
