...
### 本地优先的写作工作室：文章、作品集、文脉标本与边界清晰的 AI 辅助
...
[下载](#下载) · [截图](#截图) · [功能](#功能) · [AI-设置](#ai-设置) · [开发](#开发)

</div>

Writer 是一个安静的本地桌面写作应用：写文章，把多篇文章编成作品集，保存文脉标本，并在需要时使用 AI。它优先保护本地写作数据，AI 输出不会自动覆盖正文，需要你主动审阅和应用。

当前公开预览版是 [`tauri-mvp/`](tauri-mvp/) 下的 Tauri 版本。旧 PySide/Qt 实现仍保留在仓库里，但新的公开版本会优先围绕 Tauri 应用继续推进。
...
- 开头题记可单独编辑，导出时保持干净排版。
- 专注模式隐藏周围界面，只保留正文写作区。
- 单篇文章支持 Markdown、TXT、DOCX 导出。
- 日期页可查看每日写作记录，空日期可以直接“开始写作”。
...
- 公开预览版暂时只启用浅色模式；深色模式会在完整打磨后重新开放入口。

## 下载

从 [GitHub Releases](https://github.com/sidiangongyuan/writer/releases) 下载最新公开预览版。
