...
[![最新版本](https://img.shields.io/badge/预览版-0.1.53-2563eb?style=flat-square)](https://github.com/sidiangongyuan/living-to-tell/releases/tag/living-to-tell-v0.1.53)
...
[**下载 Windows 安装包**](https://github.com/sidiangongyuan/living-to-tell/releases/download/living-to-tell-v0.1.53/LivingToTell_0.1.53_x64-setup.exe)
 · [English](README.md)
...
| :---: | :---: | :---: | :---: | :---: |
| 在 **文章** 中起草 | 在 **作品集** 中组织书稿 | 用 **文脉与意象** 保留来源 | 使用 **AI 修改、卡片与 Agent** | 导出、检查点与备份 |
...
| 选择写作目的 | 对比 AI 结果 |
| :---: | :---: |
| ![AI 修改目的预设](tauri-mvp/docs/assets/screenshots/ai-presets.png) | ![独立 AI 结果工作区](tauri-mvp/docs/assets/screenshots/ai-results-workspace.png) |

| 选择参考上下文 | 围绕当前文章讨论 |
| :---: | :---: |
| ![文脉标本选择器](tauri-mvp/docs/assets/screenshots/ai-reference-picker.png) | ![文章 AI 对话抽屉](tauri-mvp/docs/assets/screenshots/article-ai-chat.png) |

### 把反复出现的观念整理成作者确认的关系

![意象关系星图](tauri-mvp/docs/assets/screenshots/motif-star-map.png)

<sub>在 D3 星图中缩放、聚焦和查看待丰富概念；AI 只提出候选，确认前不会建立关系或创建节点。</sub>
...
[动态教程](docs/tutorials.zh-CN.md) 用 7 段短 GIF 演示完整流程：首次使用、文章写作、作品集规划、文脉与意象、AI 设置与文章修改、作品集 Agent、导出与备份。
...
| **作品集** | 自洽的“结构容器 → 正文节点”书稿树、跟随正文的标题、未编排文章、可独立排序的看板优先级、互动教程、顺序导出和作品集 Agent。 |
...
| **AI 修改** | 绑定文章或选区的润色、改写、扩写、续写；提供 25 个写作目的、同级大选择器、明确模型选择、独立多模型结果工作区、编辑副本、段落差异和版本化写回。 |
| **文章 AI 对话** | 文章侧可关闭抽屉；关闭后保留输入、会话和等待中的回复，把回复存到其他位置仍需作者明确操作。 |
...
- 可以保存多个互相独立的 AI 配置档案，每个档案拥有自己的提供方、模型、地址、本机凭据来源和可见健康状态；单模型功能只使用唯一默认档案。
- AI 修改可以运行一个或多个明确勾选的档案；首次选择非默认档案会替换单独选中的默认项，多模型任务不会偷偷把默认档案加回来。
- 从文章精确选区进入 AI 修改时，即使文章带有题记，预览和后续写回范围也会与编辑器中真正选中的文字保持一致。
- 润色、改写、扩写和续写共提供 25 个内置写作目的。创作性预设会提示变化风险，视角、时态、论述方向和场面重心使用结构化选项，不要求作者自己拼 Prompt。
- 文脉标本、AI Cards 和当前文章便签分别使用大尺寸可搜索选择器。只有确认过的资料会附加，所有模型收到同一份冻结快照；文脉提示词明确禁止照搬原句或移植标本事实、人物和专有名词。
- 运行后进入独立结果工作区，可查看本次启动历史、原文对照、双模型、段落差异和编辑副本。展示、复制、对比和写回前会保守整理模型意外产生的连续空行，但不会破坏诗行、列表或缩进。
- 三步向导负责新建档案；先做免费的本地检查，再只对勾选档案发送最小真实请求。真实测试可能使用 token 并产生供应商费用。
...
- 推荐安装包：[`LivingToTell_0.1.53_x64-setup.exe`](https://github.com/sidiangongyuan/living-to-tell/releases/download/living-to-tell-v0.1.53/LivingToTell_0.1.53_x64-setup.exe)
- MSI 安装包：[`LivingToTell_0.1.53_x64_zh-CN.msi`](https://github.com/sidiangongyuan/living-to-tell/releases/download/living-to-tell-v0.1.53/LivingToTell_0.1.53_x64_zh-CN.msi)
...
| [AI 配置排查](docs/ai-troubleshooting.zh-CN.md) · [English](docs/ai-troubleshooting.md) | 档案健康状态、接入方式、安全诊断、协议、超时与费用行为。 |
| [动态教程](docs/tutorials.zh-CN.md) · [English](docs/tutorials.md) | 用 7 段短 GIF 看懂核心流程。 |
