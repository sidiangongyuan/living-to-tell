...
class AssignFragmentDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        form = QFormLayout()
        form.addRow(TR("assign.project_label"), self._project_combo)
        form.addRow(TR("assign.chapter_label"), self._chapter_combo)
    ...
    def _on_project_changed(self, *_args) -> None:
        ...
        self._chapter_combo.addItem(TR("assign.no_chapter"), _NO_CHAPTER_ID)
        ...
        for chapter in self._chapters.list_for_project(project_id):
            self._chapter_combo.addItem(
                chapter.title or TR("assign.untitled_chapter"), chapter.id
            )
