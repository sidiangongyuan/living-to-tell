...
def test_tauri_app_update_check_reports_latest_release(monkeypatch):
    ...
    class FakeResponse:
        ...
        def read(self):
            return json.dumps(
                {
                    "tag_name": "living-to-tell-v0.1.11",
                    "name": "Living to Tell Preview 0.1.11",
                    "html_url": "https://github.com/sidiangongyuan/living-to-tell/releases/tag/living-to-tell-v0.1.11",
                    "published_at": "2026-06-26T01:02:03Z",
                    "body": "## 0.1.11\n\nAdded update notifications.",
                    "assets": [
                        {
                            "name": "LivingToTell_0.1.11_x64_zh-CN.msi",
                            "browser_download_url": "https://example.test/LivingToTell_0.1.11_x64_zh-CN.msi",
                        },
                        {
                            "name": "LivingToTell_0.1.11_x64-setup.exe",
                            "browser_download_url": "https://example.test/LivingToTell_0.1.11_x64-setup.exe",
                        },
                    ],
                }
            ).encode("utf-8")
    ...
    assert payload["current_version"] == "0.1.10"
    assert payload["latest_version"] == "0.1.11"
    assert payload["release_name"] == "Living to Tell Preview 0.1.11"
    assert payload["download_name"] == "LivingToTell_0.1.11_x64-setup.exe"
    assert payload["download_url"] == "https://example.test/LivingToTell_0.1.11_x64-setup.exe"
